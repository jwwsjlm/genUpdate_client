...
- 并发下载时显示整体字节进度和整体下载速度，避免大文件下载期间看起来像卡住
...
Release 包会通过 GoReleaser 把 tag 版本注入到二进制里，例如 `0.5.4`。如果你自己用 `go build` 编译，没有额外传入 `-ldflags`，则显示 `dev`。
...
不一定。并发会增加服务端压力，也会让日志更密集。一般建议从 `2` 或 `3` 开始。并发模式下进度条显示的是所有文件的整体字节进度和整体下载速度；如果某个大文件还在下载，进度会继续刷新，不会只停在“已完成文件数”上。
